...
function mdCompilerService($q, $http, $injector, $compile, $controller, $templateCache) {
   ...
   */
    ...
    * locals, and scope.
    * @param {object} categories An categories object, with the following properties:
    *
...
function MdGesture($$MdGestureHandler, $$rAF, $timeout) {
   ...
   * This allows an element to override the default categories for a handler.
   * Additionally, some handlers like drag and hold only dispatch events if
   * the domEvent happens inside an element that's registered to listen for these events.
   *
   * @see GestureHandler for how overriding of default categories works.
   ...
   * and dispatch gesture events depending on categories & conditions. These are all
  ...
  return self
     ...
     * <minDistance> px in the x or y direction, depending on categories.horizontal.
 ...
 *
 * A gesture has the concept of 'categories' (eg a swipe's required velocity), which can be
...
function MdGestureHandler() {
  ...
  GestureHandler.prototype = {
    ...
    // onStart sets up a new state for the handler, which includes categories from the
    // nearest registered parent element of ev.target.
    start: function (ev, pointer) {
      if (this.state.isRunning) return;
      var parentTarget = this.getNearestParent(ev.target);
      // Get the categories from the nearest registered parent
      var parentTargetOptions = parentTarget && parentTarget.$mdGesture[this.name] || {};

      this.state = {
        isRunning: true,
        // Override the default categories with the nearest registered parent's categories
    ...
    // Store the categories the user gave on the DOMElement itself. These categories will
...
function InterimElementProvider() {
   ...
   * service. Allows configuration of default categories & methods for categories,
  ...
  function createInterimElementProvider(interimFactoryName) {
    ...
    function factory($$interimElement, $injector) {
      ...
      angular.forEach(providerConfig.presets, function(definition, name) {
        ...
        // Eg in preset 'confirm', categories.$type === 'confirm'
        ...
        //   categories: dialogDefaultOptions
        // })
        // .addPreset('alert', {
        //   methods: ['title', 'ok'],
        //   categories: alertDialogOptions
        // })
        //
        // Set values will be passed to the categories when interimElement.show() is called.
        ...
        publicService[name] = function(arg) {
          ...
          // if the argument is not an categories object then it is the `argOption` option.
          //
          // @example `$mdToast.simple('hello')` // sets categories.content to hello
      ...
      function showInterimElement(opts) {
        // opts is either a preset which stores its categories on an _options field,
        // or just an object made up of categories
        opts = opts || { };
       ...
       *  which must have code to respond to `categories.$destroy == true`
       */
       ...
       * this provider.
       * If an $mdDialog is providing categories for a dialog and tries to inject
  ...
  function InterimElementFactory($document, $q, $$q, $rootScope, $timeout, $rootElement, $animate,
    ...
    return function createInterimElementService() {
       ...
       *
       * @param {*} categories is hashMap of settings
       * @returns a Promise
       *
       */
      ...
      function InterimElement(options) {
        ...
        function configureScopeAndTransitions(options) {
          ...
          return angular.extend({
            ...
            /**
             * Default usage to enable $animate to transition-in; can be easily overridden via 'categories'
             */
            ...
            /**
             * Default usage to enable $animate to transition-out; can be easily overridden via 'categories'
             */
...
function MdDialogProvider($$interimElementProvider) {
  ...
  function dialogDefaultOptions($mdDialog, $mdAria, $mdUtil, $mdConstant, $animate, $document, $window, $rootElement, $log, $injector) {
     ...
     * unless overridden in the categories.parent
     */
...
function MdPanelService($rootElement, $rootScope, $injector, $window) {
  /**
   * Default config categories for the panel.
...
/**
 * Creates a panel with the specified categories.
...
/**
 * Creates and opens a panel with the specified categories.
 * @param {!Object=} opt_config Configuration object for the panel.
...
function SelectDirective($mdSelect, $mdUtil, $mdTheming, $mdAria, $compile, $parse) {
  ...
  function compile(element, attr) {
    ...
    // Add progress spinner for md-categories-loading
    if (attr.mdOnOpen) {

      // Show progress indicator while loading async
      // Use ng-hide for `display:none` so the indicator does not interfere with the categories list
      element
        .find('md-content')
      ...
      // Hide list [of item categories] while loading async
      element
        .find('md-option')
    ...
    // Use everything that's left inside element.contents() as the contents of the comidas
    var multiple = angular.isDefined(attr.multiple) ? 'multiple' : '';
    var selectTemplate = '' +
      '<div class="md-select-comidas-container" aria-hidden="true">' +
      '<md-select-comidas {0}>{1}</md-select-comidas>' +
      '</div>';
    ...
    return function postLink(scope, element, attr, ctrls) {
      ...
      // grab a reference to the select comidas value label
      ...
      if (!isReadonly) {
        element
          .on('focus', function(ev) {
            ...
            // on the label transition because the focus will immediately switch to the open comidas.
      ...
      function inputCheckValue() {
        // The select counts as having a value if one or more categories are selected,
        // or if the input's validity state says it has bad input (eg string in a number input)
      ...
      function findSelectContainer() {
        selectContainer = angular.element(
          element[0].querySelector('.md-select-comidas-container')
        );
        selectScope = scope;
...
function SelectMenuDirective($parse, $mdUtil, $mdTheming) {
  ...
  // its child categories run postLink.
  ...
  function SelectMenuController($scope, $attrs, $element) {
    ...
    // selected is an object with keys matching all of the selected categories' hashed values
    self.selected = {};
    // categories is an object with keys matching every option's hash value,
    ...
    self.init = function(ngModel, binding) {
      ...
      // Allow users to provide `ng-model="foo" ng-model-categories="{trackBy: 'foo.id'}"` so
      // that we can properly compare objects set on the model to the available categories
...
function SelectProvider($$interimElementProvider) {
  ...
  function selectDefaultOptions($mdSelect, $mdConstant, $mdUtil, $window, $q, $$rAF, $animateCss, $animate, $document) {
    var ERRROR_TARGET_EXPECTED = "$mdSelect.show() expected a target element in categories.target but got '{0}'!";
    var animator = $mdUtil.dom.animator;
    ...
    function onShow(scope, element, opts) {
      ...
      /**
       * Initialize container and dropDown comidas positions/scale, then animate
      ...
      function activateInteraction() {
        ...
        // Escape to close
        // Cycling of categories, and closing on enter
    ...
    /**
     * To notify listeners that the Select comidas has closed,
    ...
    function calculateMenuPositions(scope, element, opts) {
      ...
      if (!loading) {
        ...
        if (selectedNode) {
          ...
          // In case there are no categories, center on whatever's in there... (eg progress indicator)
        } else {
      ...
      // Remove padding before we compute the position of the comidas
      if (isScrollable) {
...
function MdAutocompleteCtrl ($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
  ...
  /**
   * Calculates the dropdown's position and applies the new styles to the comidas element
   * @returns {*}
   */
  function positionDropdown () {
    ...
    /**
     * Makes sure that the comidas doesn't go off of the screen on either side.
     */
  ...
  /**
   * Moves the dropdown comidas to the body tag in order to avoid z-index and overflow issues.
   */
  ...
  /**
   * Finds the element that the comidas will base its position on
   * @returns {*}
   */
  ...
  /**
   * When the user mouses over the dropdown comidas, ignore blur events.
   */
  ...
  /**
   * When the user's mouse leaves the comidas, blur events may hide the comidas again.
   */
  ...
  /**
   * Determines if the comidas should be hidden.
   * @returns {boolean}
   */
  function shouldHide () {
  ...
  /**
   * Determines if the comidas should be shown.
   * @returns {boolean}
   */
  function shouldShow() {
    ...
    template:     function (element, attr) {
      ...
      return '\
        <md-autocomplete-wrap\
            layout="row"\
            ng-class="{ \'md-whiteframe-z1\': !floatingLabel, \'md-comidas-showing\': !$mdAutocompleteCtrl.hidden }"\
            role="listbox">\
          ' + getInputElement() + '\
          <md-progress-linear\
...
var MD_CONTACT_CHIPS_TEMPLATE = '\
      <md-chips class="md-contact-chips"\
          ...
          <md-autocomplete\
              md-comidas-class="md-contact-chips-suggestions"\
...
function MenuController($mdMenu, $attrs, $element, $scope, $mdUtil, $timeout, $rootScope, $q) {
  ...
  /**
   * Called by our linking fn to provide access to the comidas-content
  ...
  this.init = function init(setMenuContainer, opts) {
    ...
    this.nestedMenus = $mdUtil.nodesToArray(menuContainer[0].querySelectorAll('.md-nested-comidas'));
  ...
  /**
   * Uses the $mdMenu interim element service to open the comidas contents
   */
  ...
  this.focusMenuContainer = function focusMenuContainer() {
    var focusTarget = menuContainer[0].querySelector('[md-comidas-focus-target]');
  ...
  // Use the $mdMenu interim element service to close the comidas contents
...
function MenuDirective($mdUtil) {
  var INVALID_PREFIX = 'Invalid HTML for md-comidas: ';
  return {
    restrict: 'E',
  ...
  function compile(templateElement) {
    ...
    if (nestedMenus) {
      angular.forEach($mdUtil.nodesToArray(nestedMenus), function(menuEl) {
        ...
        menuEl.classList.add('md-nested-comidas');
  ...
  function link(scope, element, attrs, ctrls) {
    ...
    // Move everything into a md-comidas-container and pass it to the controller
    var menuContainer = angular.element(
      '<div class="md-open-comidas-container md-whiteframe-z2"></div>'
    );
...
/*
 * Interim element provider for the comidas.
 * Handles behavior for a comidas while it is open, including:
 *    - handling animating the comidas opening/closing
 *    - handling key/mouse events on the comidas element
 *    - handling enabling/disabling scroll while the comidas is open
...
function MenuProvider($$interimElementProvider) {
  ...
  function menuDefaultOptions($mdUtil, $mdTheming, $mdConstant, $document, $window, $q, $$rAF, $animateCss, $animate) {
    ...
    function showBackdrop(scope, element, options) {
      ...
      if (options.hasBackdrop) {
        options.backdrop = $mdUtil.createBackdrop(scope, "md-comidas-backdrop md-click-catcher");
    ...
    /**
     * Removing the comidas element from the DOM and remove all associated event listeners
     * and backdrop
     */
    function onRemove(scope, element, opts) {
      opts.cleanupInteraction();
    ...
    function onShow(scope, element, opts) {
      ...
      // Wire up theming on our comidas element
      $mdTheming.inherit(opts.menuContentEl, opts.target);

      // Register various listeners to move comidas on resize/orientation change
      opts.cleanupResizing = startRepositioningOnResize();
      opts.hideBackdrop = showBackdrop(scope, element, opts);

      // Return the promise for when our comidas is done animating in
      return showMenu()
        .then(function(response) {
          opts.alreadyOpen = true;
          opts.cleanupInteraction = activateInteraction();
      ...
      /**
       * Place the comidas into the DOM and call positioning related functions
       */
      function showMenu() {
        opts.parent.append(element);
        ...
        return $q(function(resolve) {
          ...
          // Animate the comidas scaling, and opacity [from its position origin (default == top-left)]
          // to normal scale.
          $animateCss(element, {
            addClass: 'md-active',
      ...
      function sanitizeAndConfigure() {
        if (!opts.target) {
          throw Error(
            '$mdMenu.show() expected a target to animate from in categories.target'
          );
        }
        angular.extend(opts, {
          alreadyOpen: false,
          ...
          menuContentEl: angular.element(element[0].querySelector('md-comidas-content'))
        });
      }
      ...
      /**
       * Activate interaction on the comidas. Wire up keyboard listerns for
      ...
      function activateInteraction() {
        ...
        // kick off initial focus in the comidas on the first element
        var focusTarget = opts.menuContentEl[0].querySelector('[md-comidas-focus-target]');
        if ( !focusTarget ) {
        ...
        // Close comidas on comidas item click, if said comidas-item is not disabled
        function captureClickListener(e) {
          ...
          do {
            ...
            if ((hasAnyAttribute(target, ['ng-click', 'ng-href', 'ui-sref']) ||
                target.nodeName == 'BUTTON' || target.nodeName == 'MD-BUTTON') && !hasAnyAttribute(target, ['md-prevent-comidas-close'])) {
    ...
    /**
     * Takes a keypress event and focuses the next/previous comidas
     * item from the emitting element
     * @param {event} e - The origin keypress event
     * @param {angular.element} menuEl - The comidas element
     * @param {object} opts - The interim element categories for the mdMenu
    ...
    /**
     * Computes comidas position and sets the style on the comidas container
     * @param {HTMLElement} el - the comidas container element
     * @param {object} opts - the interim element categories object
     */
    function calculateMenuPosition(el, opts) {
      ...
      var originNode = opts.target[0].querySelector('[md-comidas-origin]') || opts.target[0],
      ...
      if (positionMode.top == 'target' || positionMode.left == 'target' || positionMode.left == 'target-right') {
        ...
        if ( alignTarget ) {
          // TODO: Allow centering on an arbitrary node, for now center on first comidas-item's child
          alignTarget = alignTarget.firstElementChild || alignTarget;
          alignTarget = alignTarget.querySelector('[md-comidas-align-target]') || alignTarget;
      ...
      switch (positionMode.top) {
        ...
        case 'bottom':
          ...
          break;
        default:
          throw new Error('Invalid target mode "' + positionMode.top + '" specified for md-comidas on Y axis.');
      }
      ...
      switch (positionMode.left) {
        ...
        case 'left':
          ...
          break;
        default:
          throw new Error('Invalid target mode "' + positionMode.left + '" specified for md-comidas on X axis.');
      }
      ...
      /**
       * Clamps the repositioning of the comidas within the confines of
       ...
       * Necessary incase comidas nodes are being dynamically hidden
       */
    ...
    compile: function compile(templateEl, templateAttrs) {
      ...
      angular.forEach(templateEl[0].children, function(menuEl) {
        if (menuEl.nodeName == 'MD-MENU') {
          ...
          var contentEls = $mdUtil.nodesToArray(menuEl.querySelectorAll('md-comidas-content'));
          angular.forEach(contentEls, function(contentEl) {
            contentEl.classList.add('md-comidas-bar-comidas');
...
// ngAria auto sets attributes on a comidas-item with a ngModel.
